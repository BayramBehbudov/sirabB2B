...
const SaleConditionPriceController = ({ control, formPrices }) => {
  const { t } = useTranslation();

  const { fields, append, remove } = useFieldArray({
    control,
    name: "saleConditionLines",
    keyName: "fieldId",
  });
      ...
      <div className="flex flex-col gap-2 flex-wrap">
        {fields.map((item, index) => {
          return (
            ...
            >
              <ProductHandler
                name={`saleConditionLines.${index}.productId`}
                control={control}
                disabledIds={formPrices.map((price) => price.productId)}
              />
              <Controller
                control={control}
                name={`saleConditionLines.${index}.price`}
              ...
              <ControlledSwitch
                label={t("isVAT")}
                control={control}
                name={`saleConditionLines.${index}.isVAT`}
                placeholder={t("isVAT")}
              />
...
export default SaleConditionPriceController;
